...
df_filtered = df[(df['shot_distance'] != 'error') & (df['style'] != 'error')]

# Prepare output filename
base_name = os.path.basename(csv_file)
base_name_no_ext = os.path.splitext(base_name)[0]
output_csv_file = os.path.join(output_csv_dir, f"{base_name_no_ext}_without_error.csv")
...
print(f"Rows removed (error shot_distance or style): {error_num}")
